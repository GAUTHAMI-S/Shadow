    ...
    <div>
      ...
      <div className="card animate-fade-in">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="relative">
              <FiUser size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gd-muted" />
              <input type="text" value={form.name} onChange={set('name')} placeholder="Jane Doe" required className="pl-9" />
            </div>
          </div>
          <div>
            ...
            <div className="relative">
              ...
              <input type="email" value={form.email} onChange={set('email')} placeholder="you@example.com" required className="pl-9" />
            </div>
          </div>
          <div>
            ...
            <div className="relative">
              ...
              <input type={showPass ? 'text' : 'password'} value={form.password} onChange={set('password')} placeholder="Min 8 characters" required className="pl-9 pr-9" />
              <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gd-muted hover:text-gd-text">
          ...
          <div>
            ...
            <div className="relative">
              <FiLock size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gd-muted" />
              <input type="password" value={form.confirm} onChange={set('confirm')} placeholder="Repeat password" required className="pl-9" />
            </div>
          </div>
          <button type="submit" disabled={isLoading} className="btn-primary w-full justify-center py-2.5 disabled:opacity-60">
            {isLoading ? (
        ...
        <p className="text-center text-sm text-gd-muted mt-5">
          ...
          <Link href="/login" className="text-gd-accent hover:underline">Sign in</Link>
        </p>
      </div>
    </div>
  );
